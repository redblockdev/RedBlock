...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$redblock_qt_want_version" = xauto && test "x$redblock_qt_force" != xyes; then
    if test "x$bitcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; redblock-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$bitcoin_enable_qt" != xno && test "x$redblock_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build redblock-qt GUI (default=auto)])],
    [
     redblock_qt_want_version=$withval
     if test "x$redblock_qt_want_version" = xyes; then
       redblock_qt_force=yes
       redblock_qt_want_version=auto
     fi
    ],
    [redblock_qt_want_version=auto])
...
dnl Inputs: redblock_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If redblock_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: redblock_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$redblock_qt_want_version" = xauto; then
