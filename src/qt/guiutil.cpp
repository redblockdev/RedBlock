...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a REDB address (e.g. %1)").arg(
...
{
    QString ret = QString("redblock:%1").arg(info.address);
    int paramCount = 0;
...
bool openBitcoinConf()
{
    ...
    /* Open redblock.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "REDB.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "REDB (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("REDB (%s).lnk", chain);
}
